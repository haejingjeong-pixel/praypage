...
// ui_kits/soul-pharmacy/StickerScreen.jsx — 이 파일에서 제거됨(2026-09-02). 번들에 박혀
// 있던 구버전 사본이 window.StickerScreen을 실제 로드되는 StickerScreen.jsx(스티커
// 튜토리얼 포함 최신 버전)보다 늦게/먼저 덮어써서 화면에 반영되지 않을 여지가 있었다.
...
// ui_kits/soul-pharmacy/audio-manager.js — 이 파일에서 제거됨(2026-09-01). 번들에 박혀 있던
// 구버전 사본이 window.__bgm을 먼저 선점해서, 실제로 로드되는 ui_kits/soul-pharmacy/audio-manager.js
// (BGM/효과음 음소거 지원이 추가된 최신 버전)가 `if (window.__bgm) return;` 가드에 막혀
// 절대 적용되지 못하던 버그의 원인이었다. window.__bgm은 이제 그 실제 스크립트가 전담한다.
