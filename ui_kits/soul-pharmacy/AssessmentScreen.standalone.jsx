  ...
  return (
    <div onDoubleClick={() => setSkip(true)} style={{ position: "relative", minHeight: "100%", overflowX: "hidden", background: "radial-gradient(120% 70% at 50% 0%, #FBF7F0 0%, var(--bg-page) 60%, #EDE7DE 100%)", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div aria-hidden="true" style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden", pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: pc ? "9%" : "7%", left: "50%", transform: "translateX(-50%)", display: "flex", gap: pc ? 30 : 14, filter: "blur(7px)", opacity: 0.26 }}>
          {["anxious", "broken", "compare", "waiting", "forgive", "distant", "calling", "thanks"].map((k, i) => (
            <img key={k} src={window.__resources["envFront_"+k]} alt="" draggable="false" style={{ width: pc ? 150 : 68, height: "auto", transform: `translateY(${i % 2 ? 20 : 0}px)`, userSelect: "none" }} />
          ))}
        </div>
      ...
      <div style={{ textAlign: "center", padding: pc ? "4px 24px 0" : "30px 24px 4px", position: "relative", zIndex: 1 }}>
        <div style={{ fontFamily: "var(--font-body)", fontSize: 12, letterSpacing: "0.22em", color: m.ink, marginBottom: pc ? 4 : 8 }}>오늘의 마음 접수</div>
        <p style={{ fontFamily: "var(--font-body)", fontSize: pc ? 14 : 13, color: "var(--text-muted)", margin: 0 }}>
          지금 마음에 가까운 것만 천천히 골라주세요.
        </p>
      </div>

      {/* 3-레이어 무대 — 봉투 하단은 clip으로 잘라 머문구 지점에서 끝난다 */}
      <div style={{ position: "relative", zIndex: 1, width: W, height: clipH, overflow: "hidden", margin: pc ? "-56px auto 0" : "0 auto 0" }}>
      <div style={{ position: "relative", width: W, height: stageH }}>
        ...
        <div style={{
          ...
          >
            <img src={window.__resources["envFront_"+mood]} alt="" draggable="false" style={{ display: "block", width: "100%", height: "auto", userSelect: "none" }} />
            <span
              style={{
                position: "absolute", left: 0, right: 0, top: "60%", transform: "translateY(-50%)",
                textAlign: "center", fontFamily: "var(--font-title)", fontWeight: 600,
                fontSize: Math.round(W * 0.062), lineHeight: 1.3, color: "var(--ink-900)",
