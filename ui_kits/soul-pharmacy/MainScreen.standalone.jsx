...
function MainScreen({ selected, onSelect, onNext }) {
  const ENVELOPES = [
    { key: "anxious", img: window.__resources.env_anxious, label: "불안해요" },
    { key: "broken",  img: window.__resources.env_broken,  label: "마음이\n무너졌어요" },
    { key: "compare", img: window.__resources.env_compare, label: "비교돼요" },
    { key: "waiting", img: window.__resources.env_waiting, label: "기다리기\n힘들어요" },
    { key: "forgive", img: window.__resources.env_forgive, label: "용서가\n안 돼요" },
    { key: "distant", img: window.__resources.env_distant, label: "하나님이\n멀게 느껴져요" },
    { key: "calling", img: window.__resources.env_calling, label: "책임이\n버거워요" },
    { key: "thanks",  img: window.__resources.env_thanks,  label: "감사가\n사라졌어요" },
  ];
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "clamp(14px,2.4vh,30px)", width: "100%", position: "relative", zIndex: 1, maxWidth: perRow === 2 ? ENV_W * 2 + 14 + 100 : ENV_W * 4 + 3 * 16 + 100 }}>
          {rows.map((row, i) => (
            <div key={i} style={{ position: "relative", paddingTop: 22 }}>
              <img src={window.__resources.woodBar} alt="" draggable="false" style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "auto", zIndex: 0, userSelect: "none", filter: "drop-shadow(0 10px 12px rgba(120,92,64,0.22))" }} />
...
window.MainScreenSA = MainScreen; window.MainScreen = MainScreen;
